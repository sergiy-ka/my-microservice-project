...
# Налаштування
REGION="us-west-2"
REPOSITORY_NAME="final-project-django-app"

echo "Початок очищення AWS ресурсів final-project..."
...
echo "1.1. Видалення Helm застосунків моніторингу..."
helm uninstall grafana -n monitoring 2>/dev/null || echo "Grafana Helm release не знайдено"
helm uninstall prometheus -n monitoring 2>/dev/null || echo "Prometheus Helm release не знайдено"
...
VPC_ID=$(aws ec2 describe-vpcs --filters "Name=tag:Project,Values=final-project" --region $REGION --query 'Vpcs[0].VpcId' --output text 2>/dev/null)
...
if [[ "$VPC_ID" != "None" && "$VPC_ID" != "" && "$VPC_ID" != "null" ]]; then
    ...
    echo "VPC з тегом Project=final-project не знайдено"
fi
...
BUCKET_NAME="terraform-state-bucket-final-project-sergiy-2025"
...
# Перевірка VPC (оновлено для final-project)
VPC_COUNT=$(aws ec2 describe-vpcs --filters "Name=tag:Project,Values=final-project" --region $REGION --query 'length(Vpcs)' --output text 2>/dev/null || echo "0")
