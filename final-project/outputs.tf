...
# Виведення інформації про Prometheus
...
output "prometheus_port_forward_command" {
  ...
  value       = "kubectl port-forward -n ${module.prometheus.prometheus_namespace} svc/${module.prometheus.prometheus_service_name} 9090:80"
}

output "alertmanager_service_name" {
  description = "Alertmanager service name"
  value       = module.prometheus.alertmanager_service_name
}
...
# Виведення інформації про Grafana
...
output "grafana_service_url" {
  ...
  value       = module.grafana.grafana_service_url
}
...
output "grafana_port_forward_command" {
  ...
  value       = module.grafana.grafana_port_forward_command
}

# Виведення команд доступу до моніторингу
output "monitoring_access_commands" {
  description = "Commands to access monitoring services"
  value = {
    prometheus = "kubectl port-forward -n ${module.prometheus.prometheus_namespace} svc/${module.prometheus.prometheus_service_name} 9090:80"
    grafana    = module.grafana.grafana_port_forward_command
    alertmanager = module.prometheus.alertmanager_service_name != null ? "kubectl port-forward -n ${module.prometheus.prometheus_namespace} svc/${module.prometheus.alertmanager_service_name} 9093:80" : "Alertmanager is disabled"
  }
}